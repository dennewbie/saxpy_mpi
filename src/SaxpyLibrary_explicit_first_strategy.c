    ...
    */
    ...
    if (processorID == masterProcessorID) {
        for (int i = 0; i < nProcessor; i++) {
            if (i != masterProcessorID) {
                tag = 10 + i;
                MPI_Recv((* c) + displacements[i], recvcounts[i], MPI_FLOAT, i, tag, commWorld, & status);
            }
        }

        for (int i = 0; i < arraySizeLoc; i++) * ((* c) + i) = cLoc[i];
    } else {
        tag = 10 + processorID;
        MPI_Send(cLoc, arraySizeLoc, MPI_FLOAT, masterProcessorID, tag, commWorld);
    }
